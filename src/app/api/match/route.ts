...
import { fetchSuncheonApplicableBenefits, ApplicablePolicy } from '@/lib/api';
import { getMergedNotices, CrawledItem } from '@/lib/crawler';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. 검색어 기반 스마트 후보군 사전 정렬 (관련도 높은 순으로 Gemini에 전달)
    const queryTokens: string[] = situation
      .toLowerCase()
      .split(/[\s,./?~!]+/)
    ...
    // AI 호출이 불가능하거나 API 키 부재 시 규칙 기반 폴백
    if (!apiKey) {
      const fallbackResult = ruleBasedMatch(situation, allPolicies, allNotices);
