...
[매우 중요한 지침]:
1. 일체의 이모티콘 및 이모지(아이콘, 특수 유니코드 기호 등)를 절대 사용하지 마세요. 오직 깔끔하고 정갈한 한국어 텍스트로만 작성하세요.
...
3. 구비서류(requiredDocuments) 분석 원칙 (최우선 준수):
   - [명시적 해당없음]: [정부24 민원인이 제출해야 하는 구비서류 원문]이 '해당없음'이거나 별도 제출 서류가 없다고 명시된 경우:
     반드시 requiredDocuments 배열에 ["해당없음 (온라인 신청 시 자격 자동 확인, 별도 제출 서류 없음)"] 항목 1개만 작성하세요. 절대 자의적으로 신분증 사본이나 주민등록등본 등의 서류를 추가하거나 지어내지 마세요.
   - [원문 확인된 서류]: 원문에 실제 필수 제출 서류(예: 신청서, 사업자등록증, 통장사본 등)가 명시되어 있다면, 원문에 기재된 서류들만 1~4개의 명확하고 간결한 서류명 목록으로 추출하세요.
   - [확인되지 않음 / 미기재]: 크롤링 실패, 원문에 구비서류 정보가 없거나 확인되지 않는 경우:
     절대 통상 서류(신분증, 등본 등)를 임의로 지어내지 말고, 반드시 ["공고 원문 확인 필요 (원문에 명시된 구비서류 없음 또는 상세 공고문 참조)"] 항목 1개로 작성하세요.
    ...
    try {
      ...
      if (parsed.bullets && Array.isArray(parsed.bullets) && parsed.headline) {
        ...
        if (Array.isArray(parsed.requiredDocuments) && parsed.requiredDocuments.length > 0) {
          ...
          reqDocs = ['공고 원문 확인 필요 (원문에 명시된 구비서류 없음 또는 상세 공고문 참조)'];
        }
...
function generateFallbackSummary(input: SummarizeInput, crawled?: DocumentCrawlResult): AiSummaryResult {
  ...
  if (docs.length === 0) {
    docs = ['공고 원문 확인 필요 (원문에 명시된 구비서류 없음 또는 상세 공고문 참조)'];
  }

  const isUnconfirmed = docs.some(d => d.includes('공고 원문 확인') || d.includes('확인 필요'));
  ...
  return {
    ...
    tip: isNoDoc 
      ...
      : (isUnconfirmed 
          ? '원문에 구비서류 정보가 명시되지 않았으므로 상세 신청 방법 및 제출 서류는 공고 원문 링크를 확인해 주세요.' 
